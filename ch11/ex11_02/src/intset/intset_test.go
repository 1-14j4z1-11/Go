...
func TestUsingMap(t *testing.T) {
	...
	check := func() {
		if len(expect) != set.Len() {
			t.Errorf("Unexpected Len : %d, want %d", set.Len(), len(expect))
		}
		for v, _ := range expect {
			if !set.Has(v) {
				t.Errorf("Does not has expected value : %d", v)
			}
		}
		for _, v := range set.Elems() {
			if !expect[v] {
				t.Errorf("Unexpected value exists : %d", v)
			}
		}
	}
